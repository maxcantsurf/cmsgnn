...
@jit(nopython = True)
def construct_nodes(clu_eta, clu_phi, clu_e, clu_nhit, num_clu,
    ...
    u = np.zeros((num_nodes, num_global_features))
...
@jit(nopython = True)
def construct_edges(clu_eta, clu_phi, clu_e, num_clu, 
    ...
    def get_dataset(self, entrystart, entrystop):
        ...
        for event in tqdm(range(len(self.is_e))):
            ...
            if num_clu > 0 and num_pf > 0:
                ...
                x, u = construct_nodes(clu_eta, clu_phi, clu_e, clu_nhit, num_clu,
                    pf_eta, pf_phi, pf_p, pf_pdgid, num_pf,
                    self.num_node_features, self.num_global_features)
                
                    
                edge_index, edge_attr = construct_edges(clu_eta, clu_phi,
                                                        clu_e, num_clu, 
                                                        pf_eta, pf_phi, 
                                                        pf_p, num_pf, 
